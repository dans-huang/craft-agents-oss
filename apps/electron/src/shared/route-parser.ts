...
export function parseCompoundRoute(route: string): ParsedCompoundRoute | null {
  ...
  if (first === 'settings') {
    ...
    const validSubpages: SettingsSubpage[] = ['app', 'appearance', 'input', 'workspace', 'permissions', 'labels', 'shortcuts', 'preferences', 'zendesk']
...
function convertParsedRouteToNavigationState(parsed: ParsedRoute): NavigationState | null {
  ...
  switch (parsed.name) {
    ...
    case 'zendesk':
      return { navigator: 'settings', subpage: 'zendesk' }
    case 'sources':
      return { navigator: 'sources', details: null }
    case 'source-info':
      if (parsed.id) {
